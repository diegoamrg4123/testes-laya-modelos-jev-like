...
def compose(game, decision, stats):
    ...
    c.put(1, left, f"{name.upper()}  /  LOCAL INTELLIGENCE", MUTED)
    ...
    c.put(4, right, name, GREEN)
    c.put(5, right, f"{stats.get('hardware', 'Linux')} · Local", MUTED)
    ...
    c.put(25, right + 18, stats.get("network", "offline").upper(), GREEN)
    c.put(26, right, "ENGINE", MUTED)
    c.put(26, right + 18, stats.get("engine", "PyTorch"), MUTED)
    guarded = stats.get("guarded", True)
    c.put(28, right, f"{name} + cycle safety" if guarded else f"{name} shield OFF", MUTED)
    ...
    c.put(height - 2, right, f"ESTIMATES BY {name.upper()}   {clock}", MUTED)
    return c
